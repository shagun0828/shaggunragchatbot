"""
Simple Local Scheduler for Phase 4.3 Multi-Model Ingest Pipeline
...
class SimpleSchedulerLogger:
    """Simple logger for scheduler without external dependencies"""
    ...
    def __init__(self, log_dir: str = "logs"):
        ...
        self.log_file = self.log_dir / f"simple_scheduler_{timestamp}.log"
        self.json_log_file = self.log_dir / f"simple_scheduler_{timestamp}.json"
        ...
        self.logger.info(f"Simple Scheduler Logger initialized - Log file: {self.log_file}")
    
    def _setup_logger(self) -> logging.Logger:
        """Setup basic logger"""
        logger = logging.getLogger("SimpleScheduler")
...
class SimpleLocalScheduler:
    """Simple local scheduler with comprehensive logging"""
    
    def __init__(self):
        self.logger = SimpleSchedulerLogger()
    ...
    async def run_complete_pipeline(self) -> SchedulerRun:
        ...
        run_id = f"simple_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    async def _health_check_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics = {
                ...
                "overall_health_score": health_status.get("overall_health_score", 0),
                "components_checked": len(health_status.get("components", {})),
                "healthy_components": len([
                    comp for comp in health_status.get("components", {}).values()
                    if comp.get("status") in ["healthy", "excellent", "good"]
                ]),
                "unhealthy_components": len([
                    comp for comp in health_status.get("components", {}).values()
                    if comp.get("status") not in ["healthy", "excellent", "good"]
                ])
            }
            
            # Check if critical components are healthy
            critical_components = ["chroma_cloud", "pipeline"]
            for component in critical_components:
                if component in health_status.get("components", {}):
                    comp_status = health_status["components"][component].get("status")
                    if comp_status not in ["healthy", "excellent", "good"]:
                        self.logger.logger.warning(f"Critical component {component} is {comp_status}")
                        # Don't fail the pipeline for non-critical components
            
            self.logger.logger.info("Health check completed")
            return metrics
    ...
    async def _data_ingestion_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ingestion_result = await self.pipeline.run()
            ...
            if metrics["uploaded_documents"] == 0 and metrics["scraped_articles"] > 0:
                self.logger.logger.warning("No documents were uploaded despite scraping articles")
            
            self.logger.logger.info("Data ingestion completed")
            return metrics
            
        except Exception as e:
            self.logger.logger.error(f"Data ingestion failed: {e}")
            raise
    ...
    async def _upload_verification_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if metrics["verification_failed"] > metrics["verification_passed"]:
                self.logger.logger.warning("Upload verification failed for majority of documents")
            
            self.logger.logger.info("Upload verification completed")
            return metrics
            
        except Exception as e:
            self.logger.logger.error(f"Upload verification failed: {e}")
    ...
    async def _report_generation_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not metrics["report_generated"]:
                self.logger.logger.warning("Failed to generate pipeline report")
            
            self.logger.logger.info("Report generation completed")
            return metrics
            
        except Exception as e:
            self.logger.logger.error(f"Report generation failed: {e}")
            # Don't fail the pipeline for report issues
            return {"error": str(e)}
    ...
    async def _notification_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.logger.error(f"Notification phase failed: {e}")
    ...
    def print_summary(self):
        ...
        print("\n" + "="*80)
        print("SIMPLE LOCAL SCHEDULER RUN SUMMARY")
        print("="*80)
...
async def main():
    """Main function to run simple local scheduler"""
    print("Starting Simple Local Scheduler for Phase 4.3 Multi-Model Ingest Pipeline")
    print("="*80)
